...
public class AgentGraph {
    ...
    public AgentGraph(Edge[] edges) {
        ...
        for (Edge e : edges) {
            if (!graph.containsKey(e.v1)) graph.put(e.v1, new Vertex(e.v1)); // map vertex key with vertex obj
    ...
    public void dijkstra(String startName) {
        ...
        for (Vertex v : graph.values()) {
            v.previous = v == source ? source : null;   // if v == source then v.previous  = source else v.previous = null
    ...
    private void dijkstra(final NavigableSet<Vertex> q) {
        ...
        while (!q.isEmpty()) {
            ...
            if (u.dist == Integer.MIN_VALUE) break; // we can ignore u (and any other remaining vertices) since they are unreachable
